...
const Onepoint = () => {
    ...
    const [dataonepoint, setdataonepoint] = useState({
        fx: '',
        x: 0,
    });
    ...
    const func = (x) => {
        var expr = compile(dataonepoint.fx);
        let scope = { x: parseFloat(x) };
        return expr.evaluate(scope);
    }

    function onepoint() {
        var X = parseFloat(dataonepoint.x), XOld = 0, PercentError = 100;
        var datax = [], dataerror = [];
        var n = 0;
                ...
                <div className={ROECSS.container}>
                    <div className={ROECSS.headform}>
                        <div className={ROECSS.formgroup}>
                            ...
                            <input type="text" placeholder="Enter the equation" onChange={(e) => setdataonepoint({ ...dataonepoint, fx: e.target.value })}></input>
                        </div>
                        <div className={ROECSS.row3}>
                            <div className={ROECSS.formgroup}>
                                <span class="detail">X</span>
                                <input type="number" name="xl" placeholder="Enter The X Start Value" onChange={(e) => setdataonepoint({ ...dataonepoint, x: e.target.value })}></input>
                            </div>
                        </div>
                        <div className={ROECSS.row1}>
                            <button type="button" onClick={onepoint}>
                                Calculate
                            </button>
                        </div>
